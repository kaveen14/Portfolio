...
import { Home, User, Layers, FolderKanban, Briefcase, Mail, Award, BookOpen, PlayCircle } from 'lucide-react'
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 md:hidden">
      <div
        ref={containerRef}
        className="nav-scroll flex items-center gap-1 px-3 py-2 rounded-full border border-[var(--bg-border)] shadow-2xl backdrop-blur-2xl"
        style={{ background: 'rgba(var(--color-primary-rgb), 0.03)', boxShadow: '0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05)', maxWidth: 'calc(100vw - 48px)' }}
      >
        {NAV_ITEMS.map(({ icon: Icon, label, href }) => {
          ...
          return (
            ...
            >
              {isActive ? (
                <span
                  className="flex items-center gap-2 px-3 py-2 rounded-full transition-all duration-400"
